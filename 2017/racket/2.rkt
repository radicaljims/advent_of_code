...
;; Day 2, goal 1
(define (split-tabs s)  (string-split s))
(define (split-lines s) (string-split s "\n"))

(define (splitter s) (map split-tabs (split-lines s)))

(define (matrify s)
  (map ((curry map) string->number) (splitter s)))
...
(check-equal? (matrify t0) '((5 1 9 5) (7 5 3) (2 4 6 8)))

(define (checksum xs)
  (let ([id (λ (x) x)])
    (- (argmax id xs) (argmin id xs))))

(define (goal-one m)
  (let ([sum ((curry apply) +)])
    (sum (map checksum (matrify m)))))

(check-equal? (goal-one t0) 18)

(define actual "1208	412	743	57	1097	53	71	1029	719	133	258	69	1104	373	367	365
...
(check-equal? (goal-one actual) 54426)
